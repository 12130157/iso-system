...
--sp_DiemDanh_GetHocSinhByDieuKien.sql
...
BEGIN
	...
	(E.HO+'' ''+E.TEN_LOT+'' ''+E.TEN) AS TENHOCSINH,A.TINH_TRANG AS TINH_TRANG
...
--sp_ISO_UpdateCoHieuOfChiTietKHGD.sql
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE NAME='sp_ISO_UpdateCoHieuOfChiTietKHGD')
BEGIN
	DROP PROC sp_ISO_UpdateCoHieuOfChiTietKHGD
END
GO
CREATE PROC sp_ISO_UpdateCoHieuOfChiTietKHGD  
 @ID INT,  
 @CO_HIEU INT,  
 @KQ INT OUTPUT  
AS  
BEGIN  
 SET @KQ = -1  
 IF EXISTS (SELECT * FROM CHITIETKHGD WHERE ID=@ID)  
 BEGIN  
  UPDATE CHITIETKHGD SET CO_HIEU=@CO_HIEU WHERE ID=@ID  
  SET @KQ = @ID  
 END  
END  
GO

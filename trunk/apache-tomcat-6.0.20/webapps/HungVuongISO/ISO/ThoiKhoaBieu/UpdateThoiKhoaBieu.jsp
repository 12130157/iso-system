		...
		for(var i=0;i<arrChiTiet.length;i++)
		{
			...
			if(objChiTiet.sTTNoiDung != temp)
			{
				...
				if(objChiTiet.coHieu == 0)
				{
					...
					soLyThuyet++;
					kieu[STT-1] = 'LT-' + objChiTiet.coHieu + '-' + objChiTiet.tenChuong;
				}
				else if(objChiTiet.coHieu == 1)
				{
					soThucHanh++;
					kieu[STT-1] = 'TH-' + objChiTiet.coHieu + '-' + objChiTiet.tenChuong;
	...
	else if(objMonHocTKB.kieuBienSoan == '1' || objMonHocTKB.kieuBienSoan == '0')
	{
		...
		for(var i=0;i<arrChiTiet.length;i++)
		{
			...
			if(objChiTiet.sTTNoiDung != temp)
			{
				...
				coHieu = "<input type = 'hidden' value = '" + day + "-" + STT + "-" + objChiTiet.coHieu + "-" + objChiTiet.tenChuong + "' name = 'Day" + STT + "' id = 'Day" + STT + "'/>";
			...
			if((parseInt((document.getElementById('Day' + i).value.split("-"))[1])%parseInt(document.getElementById('txtSoNoiDung').value) == 0 && so == 0) || i == STT)
			{
				...
				for(var j=x;j<=i;j++)
				{
					...
					for(var z = 1;z<=y;z++)
					{
						...
						tr.insertCell(4-dem).innerHTML = "<select name = 'Nhom" + soBuoiTongCong + "' id = 'Nhom" + soBuoiTongCong + "' " + selectNhom + disable + selectNhomHoc + ">" + option + "</select>";
...
function taoMonHoc()// tao ra mon hoc voi day du du lieu va truyen ve cu so chinh
{
	...
	if(check == false)
	{
		...
		//changePhongLyThuyet();
		//changePhongThucHanh();
		if(confirm("Bạn có chắc muốn cập nhật môn học này không ???"))

...
function taoListSuDungPhong()
{
	...
	<%
		ArrayList<ChiTietTKBModel> chiTietTKBModel = ChiTietTKBDAO.getAllSuDungPhongByNamHoc(request.getParameter("maNamHoc"),null);
...
function createTableKieuDay()// tao bang 1 de chon thu tu cac buoi hoc ly thuyet va thuc hanh, chi duoc tao ra khi kieu bien soan la 1 voi so LT va TH phai > 0
{
	...
	String tenChuong = "";
		for(int i=0;i<noiDungDCMHModelList.size();i++) {
			...
			out.print("kieu[STT-1] = 'LT-" + noiDungDCMHModelList.get(i).getCoHieu() + "-" + tenChuong + "'; }");
			out.print("else if('" + noiDungDCMHModelList.get(i).getCoHieu() + "' == '1') {");
			out.print("soThucHanh++;");
			out.print("kieu[STT-1] = 'TH-" + noiDungDCMHModelList.get(i).getCoHieu() + "-" + tenChuong + "'; ");
	...
	else if(kieuBienSoan == '1' || kieuBienSoan == '0')
	{
		...
		for(int i=0;i<noiDungDCMHModelList.size();i++) {
			...
			out.print("else{");
			out.print("day = 'TH';}}");
			out.print("coHieu = \"<input type = 'hidden' value = '\" + day + \"-" + noiDungDCMHModelList.get(i).getSoBuoi() + "-" + noiDungDCMHModelList.get(i).getCoHieu() + "-" + tenChuong + "' name = 'Day\" + STT + \"' id = 'Day\" + STT + \"'/>\";");
		...
		if(document.getElementById('KieuDay_LT' + i).checked == true)
		{
			...
			{	
				if((kieu[j].split("-"))[0] == 'LT')
				{
					...
					demLT = j + 1;
					document.getElementById('Day' + i).value = 'LT-' + i + '-' + (kieu[j].split("-"))[1] + '-' + (kieu[j].split("-"))[2];
					break;
				}
			}
			soBuoiLT++;
		...
		else
		{
			for(var j=demTH;j<kieu.length;j++)
			{
				if((kieu[j].split("-"))[0] == 'TH')
				{
					document.getElementById('STT_noi_dung' + i).value = j+1;
					demTH = j + 1;
					document.getElementById('Day' + i).value = 'TH-' + i + '-' + (kieu[j].split("-"))[1] + '-' + (kieu[j].split("-"))[2];
					break;
				}
			}
			soBuoiTH++;
...
function selectKieuDay(act) // tao bang 2
{
	...
	else
	{
		if(document.getElementById('monHoc').value != '')
		{
			...
			table.rows[0].insertCell(3).innerHTML = "Kiểu dạy";
			table.rows[0].insertCell(4).innerHTML = "Nhóm";
			table.rows[0].insertCell(5).innerHTML = "Thứ";
			table.rows[0].insertCell(6).innerHTML = "Phòng";
		}
		...
		for(var i=1;i<=STT;i++)
		{
			...
			if((parseInt((document.getElementById('Day' + i).value.split("-"))[1])%parseInt(document.getElementById('txtSoNoiDung').value) == 0 && so == 0) || i == STT)
			{
				...
				for(var j=x;j<=i;j++)
				{
					...
					if(j == x)
					{
						tr.insertCell(0).innerHTML = (soTuan + tuanBatDau - 1);
					...
					for(var z = 1;z<=y;z++)
					{
						...
						tr.insertCell(3-dem).innerHTML = "<input type = 'text' name = 'HinhThucDay" + soBuoiTongCong + "' value = '" + kieuDay + "' id = 'HinhThucDay" + soBuoiTongCong + "' readonly = 'readonly'/>";
						tr.insertCell(4-dem).innerHTML = "<select name = 'Nhom" + soBuoiTongCong + "' id = 'Nhom" + soBuoiTongCong + "' " + disable + selectNhomHoc + ">" + option + "</select>";
						tr.insertCell(5-dem).innerHTML = "<select name = 'Thu" + soBuoiTongCong + "' id = 'Thu" + soBuoiTongCong + "' " + selectThu + "><option value = '1'>Thứ hai</option><option value = '2'>Thứ ba</option><option value = '3'>Thứ tư</option><option value = '4'>Thứ năm</option><option value = '5'>Thứ sáu</option><option value = '6'>Thứ bảy</option><option value = '7'>Chủ nhật</option></select>";
						tr.insertCell(6-dem).innerHTML = "<select id = 'Phong" + soBuoiTongCong + "' name = 'Phong" + soBuoiTongCong + "'>" + phongBan + "</select><font color = 'red'></font>";
						tr.cells[6-dem].id = "tdPhong" + soBuoiTongCong;
						...
						table.rows[soBuoiTongCong].cells[0].innerHTML = table.rows[soBuoiTongCong].cells[0].innerHTML + hiddenTuan + hiddenTenChuong;	
...
function taoMonHoc()// tao ra mon hoc voi day du du lieu va truyen ve cu so chinh
{
	...
	//alert(document.getElementById('MaNamHoc').value);
							...
							for(var h =0;h<listThu.length;h++)
							{
								...
								if(document.getElementById('Thu' + i).value == objThu.thu)
								{
									...
									for(var m=0;m<listPhong.length;m++)
									{
										...
										else if(document.getElementById('giaoVien').value == objPhong.maGiaoVien) 
										{
											if(c != ">" && c != "x")
											{
...
function kiemTraTrungGioHoc()
{
	...
	for(var i=1;i<=soBuoiTongCong;i++)
	{
		...
		else
		{
			...
			{
				var phong1 = document.getElementById('Phong' + j).value;
				var tdPhong1 = document.getElementById('tdPhong' + j).innerHTML;
				var c1 = tdPhong1.charAt(tdPhong1.length-8);
				if(document.getElementById('BuoiSang' + i).checked == document.getElementById('BuoiSang' + j).checked && document.getElementById('BuoiChieu' + i).checked == document.getElementById('BuoiChieu' + j).checked)
				{
					...
					{
						if(c1 != ">" && c1 != "#") {
							tdPhong1 = tdPhong1.replace(">" + c1 + "</font>",">#</font>");
							tdPhong1 = tdPhong1.replace(">" + c1 + "</FONT>",">#</FONT>");
						}
						else {
							tdPhong1 = tdPhong1.replace("></font>",">#</font>");
							tdPhong1 = tdPhong1.replace("></FONT>",">#</FONT>");
						}
						document.getElementById('tdPhong' + j).innerHTML = tdPhong1;
						document.getElementById('Phong' + j).value = phong1;
						check = false;
						tinhTrang = false;
					}
				}
			...
			if(tinhTrang == false)
			{
				if(c != ">" && c != "#") { 
					tdPhong = tdPhong.replace(">" + c + "</font>",">#</font>");
					tdPhong = tdPhong.replace(">" + c + "</FONT>",">#</FONT>");alert(tdPhong);
				}
				else {
					tdPhong = tdPhong.replace("></font>",">#</font>");
					tdPhong = tdPhong.replace("></FONT>",">#</FONT>");

...
function mapMaLoaiByID(id)
{
	if(id=="txtDanNhap")
		...
		return "<%=Constant.MALOAI_HDHUONGDANBANDAUGV%>";
	else if(id=="txtHDTX")
		return "<%=Constant.MALOAI_HUONGDANTHUONGXUYEN%>";	
	else if(id=="txtHDBDHS")
		return "<%=Constant.MALOAI_HDHUONGDANBANDAUHS%>";
	else if(id=="txtHDTXGV")
		return "<%=Constant.MALOAI_HDHUONGDANTHUONGXUYENGV%>";
	else if(id=="txtHDTXHS")
		return "<%=Constant.MALOAI_HDHUONGDANTHUONGXUYENHS%>";
	else if(id=="txtHDKT")
		return "<%=Constant.MALOAI_HUONGDANKETTHUC%>";
	else if(id=="txtHDKTGV")
		return "<%=Constant.MALOAI_HDHUONGDANKETTHUCGV%>";
	else if(id=="txtHDKTHS")
		return "<%=Constant.MALOAI_HDHUONGDANBANDAUHS%>";
	else if(id=="txtHDRL")
		return "<%=Constant.MALOAI_HUONGDANRENLUYEN%>";
	else if(id=="txtHDRLGV")
		return "<%=Constant.MALOAI_HUONGDANRENLUYENCHUNG%>";
	else
		return "";		
}
...
function capnhatRKN(str)
{
	if(rightSpace(str)!="")
	{
		document.getElementById("actionType").value="capnhatRKNThucHanh";
		document.forms['GiaoAn'].submit();	
...
<div class = "div_pagebody">
	...
	<c:when test="${isMaCTKHGD eq true}">
		...
		<c:if test="${param.stt eq 'insert'}">
			...
			<c:set var="soPhut" value='<%=KeHoachGiangDayDAO.calSoPhutDCMHByMaKHGD(ChiTietKHGDDAO.getMaKHGDByMaCTKHGD(request.getParameter("maCTKHGD")),Constant.phutQDTH)%>'></c:set>
	...
	<form action="<%=request.getContextPath()%>/GiaoAnController" method="post" name="GiaoAn">
		...
		<table border = "1" style="background-color: transparent;">
			...
			<tr style="background-color: transparent;">
				...
				<br />
					<textarea  rows="2" cols="20" style="background-color: transparent;" id="txtDanNhap" name="txtDanNhap"  readonly="readonly" onclick="showPopUp(this.id)">${sf:appendBrTag2(giaoAnLyThuyet.danNhap)}</textarea>
				</td>
				<td>
			...
			<tr>
				...
				<td>Hướng Dẫn Ban Đầu : <a class='error' id='aHDBD'></a><br />
					<textarea  rows="2" cols="20" <c:if test="${giaoAnLyThuyet.huongDanBanDau eq ''}">style="background-color: red;cursor:pointer"</c:if> <c:if test="${giaoAnLyThuyet.huongDanBanDau ne ''}">style="background-color: transparent;cursor:pointer;"</c:if> onclick="showPopUp(this.id)" readonly="readonly" id ="txtHDBD" name="txtHDBD" ><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.huongDanBanDau)}</c:if></textarea>
				</td>
				<td>
					<a class='error' id='aHDBDGV'></a>
					<textarea  rows="2" cols="20" style="background-color: transparent;cursor:pointer;" onclick="showPopUp(this.id)" readonly="readonly" id="txtHDBDGV" name="txtHDBDGV"><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.HDHDBDGV)}</c:if></textarea>
				</td>
				<td>
					<a class='error' id='aHDBDHS'></a>
					<textarea  rows="2" cols="20"style="background-color: transparent;cursor:pointer;" onclick="showPopUp(this.id)" readonly="readonly" id="txtHDBDHS" name="txtHDBDHS"><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.HDHDBDHS)}</c:if></textarea>
				</td>
				<td>
			...
			<tr>
				...
				<td>Hướng dẫn thường xuyên : <a class='error' id='aHDTX'></a><br />
					<textarea  rows="2" cols="20" id="txtHDTX" name="txtHDTX" style="cursor:pointer;background-color: transparent;" readonly="readonly" onclick="showPopUp(this.id)">${sf:appendBrTag2(giaoAnLyThuyet.huongDanThuongXuyen)}</textarea>
				</td>
				<td>
					<a class='error' id='aHDTXGV'></a>
					<textarea  rows="2" cols="20" style="background-color: transparent;cursor:pointer;" onclick="showPopUp(this.id)" readonly="readonly" id="txtHDTXGV" name="txtHDTXGV"><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.HDHDTXGV)}</c:if></textarea>
				</td>
				<td>
					<a class='error' id='aHDTXHS'></a>
					<textarea  rows="2" cols="20" style="background-color: transparent;cursor:pointer;" onclick="showPopUp(this.id)" readonly="readonly" id="txtHDTXHS" name="txtHDTXHS"><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.HDHDTXHS)}</c:if></textarea>
				</td>
				<td>
			...
			<tr>
				...
				<td>Hướng dẫn kết thúc : <a class='error' id='aHDKT'></a><br />
					<textarea  rows="2" cols="20" id ="txtHDKT" name="txtHDKT" onclick="showPopUp(this.id)" readonly="readonly" <c:if test="${giaoAnLyThuyet.huongDanKetThuc eq ''}">style="background-color: red;cursor:pointer"</c:if> <c:if test="${giaoAnLyThuyet.huongDanKetThuc ne ''}">style="background-color: transparent;cursor:pointer;"</c:if>><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.huongDanKetThuc)}</c:if></textarea>
				</td>
				<td>
					<a class='error' id='aHDKTGV'></a>
					<textarea  rows="2" cols="20" style="background-color: transparent;cursor:pointer;" id="txtHDKTGV" onclick="showPopUp(this.id)" readonly="readonly" name="txtHDKTGV"><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.HDHDKTGV)}</c:if></textarea>
				</td>
				<td>
					<a class='error' id='aHDKTHS'></a>
					<textarea  rows="2" cols="20"style="background-color: transparent;cursor:pointer;" id="txtHDKTHS" onclick="showPopUp(this.id)" readonly="readonly" name="txtHDKTHS" ><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.HDHDKTHS)}</c:if></textarea>
				</td>
				<td>
			...
			<tr>
				...
				<td>Hướng dẫn rèn luyện : <a class='error' id='aHDRL'></a><br />
					<textarea onclick="showPopUp(this.id)" readonly="readonly" rows="2" cols="20" id ="txtHDRL" name="txtHDRL" <c:if test="${giaoAnLyThuyet.huongDanRenLuyen eq ''}">style="background-color: red;cursor:pointer"</c:if> <c:if test="${giaoAnLyThuyet.huongDanRenLuyen ne ''}">style="background-color: transparent;cursor:pointer;"</c:if>><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.huongDanRenLuyen)}</c:if></textarea></td>
				<td colspan="2">
					<a class='error' id='aHDRLGV'></a>
					<textarea  rows="2" cols="50" style="background-color: transparent;cursor:pointer;" onclick="showPopUp(this.id)" readonly="readonly" id ="txtHDRLGV" name="txtHDRLGV"><c:if test="${param.stt eq 'update'}">${sf:appendBrTag2(giaoAnLyThuyet.HDHDRLGV)}</c:if></textarea>
				</td>
				<td>
		...
		<table style="background-color: transparent;">
			...
			<tr>
				<td>
					...
					<img src="<%=request.getContextPath()%>/images/buttom/rutkinhnghiem.png" alt='Rút kinh nghiệm' style="cursor:pointer;" onclick="capnhatRKN(document.getElementById('txtRutKinhNghiem').value)"/>
				</td>
			</tr>
		</table>
		<table style="background-color: transparent;">
			<tr style="background-color: transparent;">
				...
				<td>
					<input type = "hidden" id="txtCalendar1" size = 8 name="txtCalendar1" <c:if test="${param.stt eq 'update'}">value="${giaoAnLyThuyet.ngayThucHien}"</c:if> readonly="readonly" />

...
public class DeNghiKTVController extends HttpServlet{
	...
	private void SaveDeNghiKTV(HttpServletRequest request,HttpServletResponse response){
		...
		try {
			...
			RequestDispatcher rd = request.getRequestDispatcher("NhanSu/DeNghiKhoanThuViec/DeNghiKhoanThuViec.jsp?id="+request.getParameter("txtMaDeNghi"));
	...
	private void SendDeNghiKTV(HttpServletRequest request,HttpServletResponse response){
		...
		String truongkhoa = DeNghiKhoanThuViecDAO.getTK(kq);
		kq = DeNghiKhoanThuViecDAO.approveDeNghiKTV(request.getParameter("txtMaDeNghi"), truongkhoa);
		DeNghiKTVModel DNKTV = DeNghiKhoanThuViecDAO.getDeNghiKTVByID(request.getParameter("txtMaDeNghi"));
		String test = DNKTV.getTen_nguoi_du_tuyen();
		if(kq != 0){
	...
	private void ApproveDeNghiKTV(HttpServletRequest request,HttpServletResponse response){
		...
		if(kq!=0){
			...
			if(DNKTV.getTinh_trang().equals("3")){
				mailto = MailDAO.getMailListByMaBoPhan(Constant.BO_PHAN_PHC);
			} else if(DNKTV.getTinh_trang().equals("4")){
				...
				HoSoDuTuyenModel HSDT = HoSoDuTuyenDAO.getHoSoByID(DNKTV.getNguoi_du_tuyen());
			}

...
private static final long serialVersionUID = 1L;
	...
	private void duyetMotDeCuongMonHoc(HttpServletRequest request,
		...
		if(request.getParameter("Duyet").equals("Approve")) {
			...
			MailUtil.sendEmail(	MailDAO.getMailByMaThanhVien(deCuongMonHocModel.getMaNguoiTao()),
									MailDAO.getMailListByMaVaiTro(Constant.TRUONG_KHOA),
		...
		if (request.getParameter("GuiTruongKhoa") != null
			...
			if (deCuongMonHocModel.validateModel()) { 
				...
				if (DeCuongMonHocDAO.updateDeCuongMonHoc(deCuongMonHocModel)) {
					...
					MailUtil.sendEmail(	MailDAO.getMailListByMaVaiTro(Constant.TRUONG_KHOA),
											MailDAO.getMailByMaThanhVien(deCuongMonHocModel.getMaNguoiTao()),

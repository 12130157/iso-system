...
public class HopDongLanDauDAO {
	public static int insertHopDongLanDau(String idNguoiDuTuyen,String tinhTrang,String maTruongKhoa){
		int kq = 0;
		try {
	...
	public static int updateHopDongLanDau(String id,String thoigian,String batdau,String ketthuc,String luong,String tinhtrang){
		int kq = 0;
		try {
	...
	public static int approveHopDongLanDau(String id,String manguoiduyet){
		int kq = 0;
		try {
	...
	public static int rejectHopDongLanDau(String id,String lyDoReject){
		int kq = 0;
		try {
	...
	public static HopDongLanDauModel getHopDongLanDauByID(String id){
		HopDongLanDauModel model = null;
		try {
			String sql = "SELECT A.*, "
						...
						+" FROM HopDongLanDau A INNER JOIN HOSODUTUYEN B ON A.NGUOI_DU_TUYEN=B.ID "
	...
	public static ArrayList<HopDongLanDauModel> getAllHopDongLanDau(String maBoPhan,String maTruongKhoa,String tinhTrang){
		ArrayList<HopDongLanDauModel> list = new ArrayList<HopDongLanDauModel>();
		try {
			CallableStatement csmt = DataUtil.getConnection().prepareCall("{call sp_NhanSu_SelectAllHopDongLanDau(?,?,?)}");
			csmt.setString("Ma_bo_phan", maBoPhan);
			csmt.setString("Ma_truong_khoa", maTruongKhoa);
			csmt.setString("Tinh_trang", tinhTrang);
			ResultSet rs = csmt.executeQuery();
			while(rs.next()){
				HopDongLanDauModel model = new HopDongLanDauModel();
				model.setId(rs.getString("ID"));
				model.setNguoi_du_tuyen(rs.getString("Nguoi_du_tuyen"));
				model.setTruong_khoa(rs.getString("Ten_nguoi_lap"));
				model.setNgay_lap_mdy(rs.getString("Ngay_lap_mdy"));
				model.setTinh_trang(rs.getString("Tinh_trang"));
				model.setLy_do_reject(rs.getString("Ly_do_reject"));
				model.setTen_nguoi_du_tuyen(rs.getString("TEN_NGUOI_DU_TUYEN"));
	...
	public static HopDongLanDauModel getHopDongLanDauByTenDangNhap(String tenDangNhap){
		HopDongLanDauModel model = null;
		try {

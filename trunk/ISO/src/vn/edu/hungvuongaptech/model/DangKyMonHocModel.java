package vn.edu.hungvuongaptech.model;

import java.util.ArrayList;

public class DangKyMonHocModel {

	private String maDangKyMonHoc;
	private String maMonHocTKB;
	private String maHocVien;
	private String diemTrungBinh;
	private String hanhKiem;
	private String hocLuc;
	private String danhHieu;
	private String ghiChu;
	private String ngayCapNhatCuoi;
	private String user1;
	private String user2;
	private String user3;
	private String user4;
	private String user5;
	private ArrayList<ChiTietDiemModel> chiTietDiemList;
	private String tenMonHoc;
	private String tenDangNhap;
	
	//
	private String maSoDiemMonHoc;
	private String maMonHoc;
	
	public String getMaSoDiemMonHoc() {
		return maSoDiemMonHoc;
	}
	public void setMaSoDiemMonHoc(String maSoDiemMonHoc) {
		this.maSoDiemMonHoc = maSoDiemMonHoc;
	}
	public String getMaMonHoc() {
		return maMonHoc;
	}
	public void setMaMonHoc(String maMonHoc) {
		this.maMonHoc = maMonHoc;
	}
	//
	public String getTenDangNhap() {
		return tenDangNhap;
	}
	public void setTenDangNhap(String tenDangNhap) {
		this.tenDangNhap = tenDangNhap;
	}
	private String tenHocVien;
	
	public String getTenMonHoc() {
		return tenMonHoc;
	}
	public void setTenMonHoc(String tenMonHoc) {
		this.tenMonHoc = tenMonHoc;
	}
	public String getTenHocVien() {
		return tenHocVien;
	}
	public void setTenHocVien(String tenHocVien) {
		this.tenHocVien = tenHocVien;
	}
	public ArrayList<ChiTietDiemModel> getChiTietDiemList() {
		return chiTietDiemList;
	}
	public void setChiTietDiemList(ArrayList<ChiTietDiemModel> chiTietDiemList) {
		this.chiTietDiemList = chiTietDiemList;
	}
	public String getMaDangKyMonHoc() {
		return maDangKyMonHoc;
	}
	public void setMaDangKyMonHoc(String maDangKyMonHoc) {
		this.maDangKyMonHoc = maDangKyMonHoc;
	}
	public String getMaMonHocTKB() {
		return maMonHocTKB;
	}
	public void setMaMonHocTKB(String maMonHocTKB) {
		this.maMonHocTKB = maMonHocTKB;
	}
	public String getMaHocVien() {
		return maHocVien;
	}
	public void setMaHocVien(String maHocVien) {
		this.maHocVien = maHocVien;
	}
	public String getDiemTrungBinh() {
		return diemTrungBinh;
	}
	public void setDiemTrungBinh(String diemTrungBinh) {
		this.diemTrungBinh = diemTrungBinh;
	}
	public String getHanhKiem() {
		return hanhKiem;
	}
	public void setHanhKiem(String hanhKiem) {
		this.hanhKiem = hanhKiem;
	}
	public String getHocLuc() {
		return hocLuc;
	}
	public void setHocLuc(String hocLuc) {
		this.hocLuc = hocLuc;
	}
	public String getDanhHieu() {
		return danhHieu;
	}
	public void setDanhHieu(String danhHieu) {
		this.danhHieu = danhHieu;
	}
	public String getGhiChu() {
		return ghiChu;
	}
	public void setGhiChu(String ghiChu) {
		this.ghiChu = ghiChu;
	}
	public String getNgayCapNhatCuoi() {
		return ngayCapNhatCuoi;
	}
	public void setNgayCapNhatCuoi(String ngayCapNhatCuoi) {
		this.ngayCapNhatCuoi = ngayCapNhatCuoi;
	}
	public String getUser1() {
		return user1;
	}
	public void setUser1(String user1) {
		this.user1 = user1;
	}
	public String getUser2() {
		return user2;
	}
	public void setUser2(String user2) {
		this.user2 = user2;
	}
	public String getUser3() {
		return user3;
	}
	public void setUser3(String user3) {
		this.user3 = user3;
	}
	public String getUser4() {
		return user4;
	}
	public void setUser4(String user4) {
		this.user4 = user4;
	}
	public String getUser5() {
		return user5;
	}
	public void setUser5(String user5) {
		this.user5 = user5;
	}

}

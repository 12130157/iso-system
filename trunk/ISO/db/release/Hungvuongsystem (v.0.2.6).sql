...
CREATE TABLE [ChuongTrinhDaoTao] (
    ...
    [Thi_hoc_ki] NVARCHAR(10),
    [Thi_tot_nghiep] NVARCHAR(10),
    [Thuc_tap] NVARCHAR(10),
    [Y_kien_de_xuat] NVARCHAR(10),
    [Ngay_tao] DATETIME,
    [Ngay_cap_nhat_cuoi] DATETIME,
    [Ma_nguoi_tao] INTEGER,
    [Ma_nguoi_duyet] INTEGER,
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW [`hungvuongsystem`.`view_tdtb`] AS select `a`.`matb` AS `matb`,`a`.`tentb` AS `tentb`,`a`.`vitri` AS `vitri`,`a`.`tinhtrang_tb` AS `tinhtrang_tb`,`a`.`tinhtrang_muon` AS `tinhtrang_muon`,`b`.`thoigian_bh` AS `thoigian_bh`,`d`.`tenphong` AS `tenphong`,`a`.`solan_bt` AS `solan_bt`,`b`.`ngay_bdsd` AS `ngay_bdsd`,`e`.`mapb` AS `mapb`,`d`.`maphong` AS `maphong`,`b`.`dtkt` AS `dtkt`,`a`.`diengiai` AS `diengiai` from ((((`hungvuongsystem`.`thietbi` `A` join `hungvuongsystem`.`chitiet_tb` `B` on((`a`.`matb` = `b`.`matb`))) join `hungvuongsystem`.`phong` `D` on((`a`.`maphong` = `d`.`maphong`))) join `hungvuongsystem`.`phongban` `E` on((`d`.`mapb` = `e`.`mapb`))) left join `hungvuongsystem`.`chitietnhatky` `C` on((`a`.`matb` = `c`.`matb`)))
GO

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW [`hungvuongsystem`.`view_tdtb_second`] AS select `a`.`matb` AS `matb`,`a`.`tentb` AS `tentb`,`a`.`vitri` AS `vitri`,`a`.`tinhtrang_tb` AS `tinhtrang_tb`,`a`.`tinhtrang_muon` AS `tinhtrang_muon`,`b`.`thoigian_bh` AS `thoigian_bh`,`d`.`tenphong` AS `tenphong`,`a`.`solan_bt` AS `solan_bt`,`b`.`ngay_bdsd` AS `ngay_bdsd`,`e`.`mapb` AS `mapb`,`d`.`maphong` AS `maphong`,`b`.`dtkt` AS `dtkt`,`a`.`diengiai` AS `diengiai` from ((((`hungvuongsystem`.`thietbi` `A` join `hungvuongsystem`.`chitiet_tb` `B` on((`a`.`matb` = `b`.`matb`))) join `hungvuongsystem`.`phong` `D` on((`a`.`maphong` = `d`.`maphong`))) join `hungvuongsystem`.`phongban` `E` on((`d`.`mapb` = `e`.`mapb`))) left join `hungvuongsystem`.`chitietnhatky` `C` on((`a`.`matb` = `c`.`matb`)))
GO

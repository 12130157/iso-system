...
BEGIN
	...
	if(@Tinh_trang <> '')
	BEGIN
		SET @Dieu_kien_tinh_trang_1 = ' CTDT.Tinh_trang = ' + @Tinh_trang
	END
	if(@Ma_bo_phan <> '')
	BEGIN
		...
		BEGIN
			SET @Dieu_kien_tinh_trang_2 = ' CTDT.Tinh_trang <> 0 '
		END
	END
	--if(@Ma_nguoi_tao <> '')
	--BEGIN
...
--INNER JOIN ChiTietThanhVien As C on B.Ten_DN = C.Ten_dang_nhap
--exec sp_ISO_GetChuongTrinhDaoTao 3,1,1,'',0,5
GO
...
BEGIN
	...
	SET @Dieu_kien_bo_phan = ''
	SET @Vai_tro_truong_bo_phan = ''
	if((@Tinh_trang <> '' and @Ma_nguoi_tao <> '') or (@Tinh_trang <> '' and @Ma_bo_phan <> ''))
	BEGIN
		SET @And = ' and '
	END
	...
	--PRINT @sql	
END
-- exec sp_ISO_GetCountDeCuongMonHoc '',20,6,5,5
...
BEGIN
	...
	BEGIN
		SET @Where = 'Where '
		SET @Dieu_kien_tinh_trang_1 = ' DCMH.Tinh_trang = ' + @Tinh_trang
	END
	...
	BEGIN
		if(@Ma_bo_phan <> '')
		BEGIN
			SET @Dieu_kien_tinh_trang_2 = ' DCMH.Tinh_trang <> 0 '
			SELECT @Vai_tro_truong_bo_phan = B.Ma_vai_tro FROM Khoa_trungtam AS A INNER JOIN ThanhVien AS B ON A.Ma_truong_khoa = B.ID WHERE B.Ma_bo_phan = @Ma_bo_phan
			if(@Vai_tro_truong_bo_phan = @Ma_truong_khoa)
			BEGIN
				SET @Dieu_kien_bo_phan = ' B.Ma_bo_phan = ' + @Ma_bo_phan + @And + @Dieu_kien_tinh_trang_2
			END
		END
	END
	--SELECT @sql = ''
	...
	--print @sql
END
--exec sp_ISO_GetDeCuongMonHoc 3,10,1,'',20,6,5,5
GO
...
BEGIN
	...
	if(@Nam_hoc <> '')
	BEGIN
		SET @Dieu_kien_nam_hoc = ' A.Ma_nam_hoc = ' + @Nam_hoc
	END

	if(@Tuan <> '')
	BEGIN
		SET @Dieu_kien_tuan = ' A.STT_Tuan = ' + @Tuan
	END
	else
	BEGIN
		SET @Dieu_kien_tuan = ' (convert(varchar(10),G.Tu_ngay,110) <= convert(varchar(10),GETDATE(),110) AND convert(varchar(10),G.Den_ngay,110) >= convert(varchar(10),GETDATE(),110)) '
	END
	...
	FROM SuDung AS A 
		...
		+ @Where + @Dieu_kien_phong + @And + @Dieu_kien_nam_hoc + @And + @Dieu_kien_tuan + @And + @Dieu_kien_khoa +
...
--sp_QLTB_GetNhaCungCapByTen.sql
...
--sp_QLTB_GetTotalNhaCungCapByTen.sql
...
--sp_QLTB_InsertChiTietThietBi.sql
...
--sp_QLTB_UpdateNhaCungCapByID.sql

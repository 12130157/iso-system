...
<head>
<%@page import="vn.edu.hungvuongaptech.model.ChiTietKHGDModel"%><html>
...
<div class = "div_pagebody">
	...
	<c:when test="${isMaCTKHGD eq true}">
		...
		<c:if test="${param.stt eq 'update'}">
			...
			<% giaoAnModel=GiaoAnDAO.getGiaoAnTichHopByMaGA(maGA); %>	
			...
			<c:set var="soPhut" value='<%=KeHoachGiangDayDAO.calSoPhutDCMHByMaKHGD(maKHGD,Constant.phutQDTHop)%>' scope="session"></c:set>
